"""
Dataset loader with prompt sampling from the full FineWeb dataset.

Each eval epoch uses random function into a different region of the
...
def sample_prompts_from_dataset(
    ...
    """
    Sample n prompts directly from the full dataset, seeded by seed.

    Uses seed to compute a skip offset into the streaming dataset,
    so each epoch draws from a completely different region of the 1.5T token
    corpus. No fixed pool — cannot predict or overfit to the prompts.

    Results are cached per seed so repeated calls (e.g. retries) return the
    same prompts.
    """
    if cache_dir is None:
        cache_dir = PROMPT_CACHE_DIR

    # Check seed-specific cache
    cache_path = cache_dir / f"seed_number_{seed}_{n}.json"
    if cache_path.exists():
        try:
            cached = json.loads(cache_path.read_text())
            if len(cached) >= n:
                logger.info(f"Using cached prompts for seed number {seed}")
                return cached[:n]
        except Exception:
            pass
    ...
    # Use seed to seed a deterministic but unpredictable prompt selection.
    # We use the seed as both the shuffle seed and a skip offset so each epoch
    # draws from a different region of the dataset.
    seed_hash = hashlib.sha256(str(seed).encode()).hexdigest()
    skip_offset = int(seed_hash[:8], 16) % 50_000  # moderate skip to avoid slow seeks
    ...
    ds = load_dataset(dataset_name, split=split, streaming=True, name="default")
    ds_shuffled = ds.shuffle(seed=seed, buffer_size=5_000)
...
def sample_prompts_seeded(
    ...
    """
    Sample n prompts from a pre-loaded pool, seeded by seed.
...
def sample_prompts(prompts: list[str], n: int) -> list[str]:
    """Random sample (for testing/simulation)."""
